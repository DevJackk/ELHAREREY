...
elharerey = Token:match("(%d+)")
redis:set(elharerey..":VERSION",1)
redis:set(elharerey..":SUDO_ID:",SUDO_USER)
redis:set(elharerey..":DataCenter:",'German')
redis:set(elharerey..":UserNameBot:",BOT_User)
redis:set(elharerey..":NameBot:",BOT_NAME)
redis:hset(elharerey..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3elharerey_INSTALL","Yes")
info = {} 
...
print('\27[0;33m>>'..[[
echo -e "\e[38;5;77m       •{ تنصيب سورس 𝑬𝑳𝑯𝑨𝑹𝑬𝑹𝒀➺𝑨𝑩. بنسبه 100% ]  }•\e[0m"
echo -e "\e[38;5;77m       •{ •{ Join the Source Channel : @el_harere }• }•\e[0m"
...
echo -e "\e[38;5;77m       •{ SOURCE ELHAREREY source has been installed }•\e[0m"
echo -e "\e[38;5;77m       •{ developer JACK : @PHP57  }•\e[0m"

echo -e "\e[38;5;77m      ELHAREREY ✾ @PHP57e[0m"
...
elharerey = Token:match("(%d+)")
our_id = tonumber(elharerey)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(elharerey..":UserNameBot:")
SUDO_ID = tonumber(redis:get(elharerey..":SUDO_ID:"))
SUDO_USER = redis:hgetall(elharerey..'username:'..SUDO_ID).username
version = redis:get(elharerey..":VERSION")
DataCenter = redis:get(elharerey..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..elharerey..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(elharerey..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(elharerey..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(elharerey..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 902921649 then 
msg.TheRank =  '𝐀𝐇𝐌𝐄𝐃 𝐁𝐀𝐃𝐑❃'
msg.Rank = 1
...
elseif msg.sender_user_id_ == 1487613664 then 
msg.TheRank =  ' 𝐄𝐋𝐇𝐀𝐑𝐄𝐑𝐄𝐘𖣍 '
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(elharerey..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(elharerey..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(elharerey..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(elharerey..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(elharerey..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(elharerey..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(elharerey..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(elharerey..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(elharerey..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(elharerey..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(elharerey..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(elharerey..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس 𝑬𝑳𝑯𝑨𝑹𝑬𝑹𝒀➺𝑨𝑩. ⇐ @el_harere
...
if msg.GroupActive and redis:get(elharerey..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(elharerey..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(elharerey..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(elharerey..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.elharerey then 
if msg.text and ScriptFile.ielharerey then
for k, elharerey in pairs(ScriptFile.elharerey) do
local SearchText = msg.text:match(elharerey)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ielharerey(msg,{msg.text:match(elharerey)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",elharerey.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.delharerey then
if ScriptFile.delharerey(msg) == false then
return false
end
...
if Plug.elharerey then 
if msg.text and Plug.ielharerey then
for k, elharerey in pairs(Plug.elharerey) do
local SearchText = msg.text:match(elharerey)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(elharerey..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(elharerey..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ielharerey(msg,{msg.text:match(elharerey)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",elharerey.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.delharerey then
Plug.delharerey(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source elharerey")
end 

end
else
print("The File Script.lua Not Runing in The Source elharerey")
...
function tdcli_update_callback(data)
	...
	redis:srem(elharerey..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELHAREREY/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURC')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELHAREREY/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELHAREREY/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE ELHA')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELHAREREY/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELHAREREY/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/ELHAREREY/main/plugins/games.lua','./plugins/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'SOURCE 𝑬𝑳𝑯𝑨𝑹𝑬𝑹𝒀➺𝑨𝑩.\n\n• ** السورس الى اصدار \n• ** تم اعاده تشغيل سورس 𝑬𝑳𝑯𝑨𝑹𝑬𝑹𝒀➺𝑨𝑩. بنجاح')
	dofile("./inc/Run.lua")
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(elharerey..":SUDO_ID:")
redis:hset(elharerey..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(elharerey..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(elharerey..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(elharerey..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(elharerey..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(elharerey..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(elharerey..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(elharerey..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(elharerey..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(elharerey..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(elharerey..'group:add'..data.chat_.id_) then
	redis:set(elharerey..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(elharerey..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(elharerey..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(elharerey..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	elharerey..'group:name'..k,IDS.Title,
	elharerey..'num_msg_elharerey'..k,5,
	elharerey..'group:add'..k,true,
	elharerey..'lock_link'..k,true,
	elharerey..'lock_spam'..k,true,
	elharerey..'lock_webpage'..k,true,
	elharerey..'lock_markdown'..k,true,
	elharerey..'lock_flood'..k,true,
	elharerey..'lock_bots'..k,true,
	elharerey..'mute_forward'..k,true,
	elharerey..'mute_contact'..k,true,
	elharerey..'mute_document'..k,true,
	elharerey..'mute_inline'..k,true,
	elharerey..'lock_username'..k,true,
	elharerey..'replay'..k,true
	)
	redis:sadd(elharerey..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(elharerey..'username:'..ID,'username',user)
	redis:sadd(elharerey..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(elharerey..'username:'..ID,'username',user)
	redis:sadd(elharerey..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(elharerey..'username:'..ID,'username',user)
	redis:sadd(elharerey..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(elharerey..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(elharerey..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(elharerey..'username:'..data.user_.id_).username
	...
	redis:hset(elharerey..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
